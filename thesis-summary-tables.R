...
#problem, for people who indicated no change, but previously indicated they would work the same amount, what__income is Na when it shouldn't be
#if what_ubi_500= "work the same amount" AND what_income_500 = NA => substitute with value from what_income_300 (same for 800 and 300)

if (dat$ubi_500 == "Work the same amount as now"){
    dat$what_income_500[is.na(dat$what_income_500)] <- dat$what_income_300[is.na(dat$what_income_500)]
}
dat_add_income_500 <- dat %>% 
  filter(!is.na(what_income_500))

tab_add_income_500 <- table1(~ what_income_500, data = dat_add_income_500)
t1flex(tab_add_income_500) %>% 
  save_as_docx(path="tab_add_income_500.docx")

if (dat$ubi_800 == "Work the same amount as now"){
  dat$what_income_800[is.na(dat$what_income_800)] <- dat$what_income_500[is.na(dat$what_income_800)]
}

dat_add_income_800 <- dat %>% 
  filter(!is.na(what_income_800))

tab_add_income_800 <- table1(~ what_income_800, data = dat_add_income_800)
t1flex(tab_add_income_800) %>% 
  save_as_docx(path="tab_add_income_800.docx")

#now do the same for "what_time" but time has responses from both work less and stop working, so create separate ones? 
#Work less 
...
#this should be 27 from ubi_500 + the ones from ubi_300 == Work less & change_500 == 0 
