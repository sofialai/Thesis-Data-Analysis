...
table1(~ ubi_300 + ubi_500 + ubi_800 | hours, data = dat_1)
...
output_800 <- lm(ubi_800 ~ hours + income_cat + age + job_satisfied + job_interesting + job_repetitive + job_boring, data = dat_1)
tbl_regression(output_800, include = c(hours, income_cat),
               label = list(hours ~ "Weekly work hours",
                            income_cat ~ "Income category")) %>%
  bold_labels() %>%
  italicize_levels() %>% 
...
#behavior under UBI = 300 and job scales
